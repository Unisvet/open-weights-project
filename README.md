...
Dieses Repository enthält den interaktiven Begleit-Leitfaden und die praktischen Entwicklungsmaterialien für den **Workshop zur digitalen Souveränität in kleinen und mittleren Unternehmen (KMU)**. Unser Ziel ist es, Betrieben im Spreeland zu zeigen, wie sie modernste Open-Weights-Sprachmodelle wie **Gemma 4** vollständig offline, datenschutzkonform und kostengünstig in ihre Produktions- und Verwaltungsprozesse integrieren können.
...
3. **Praktische Labore**:
   - **Labor 1: Gemma 4 Feinabstimmung (Fine-Tuning)**: Ein direktes Google Colab-Notebook und lokale Skripte, um Gemma mit Unsloth und QLoRA auf eigene Maschinendaten anzupassen. Enthält einen Live-Vergleichs-Simulator im Browser.
   - **Labor 2: Autonome Agenten**: Ein lauffähiger ReAct-Agent, der selbstständig logische Schlüsse zieht, interne Lagerbestände abfragt und Produktionszeiten kalkuliert. Ein Live-Agenten-Terminal im Browser simuliert den Ablauf Schritt für Schritt.
...
### 2. Labor 1: Gemma 4 Feinabstimmung (Fine-Tuning)
Das Feinabstimmungs-Notebook zeigt Ihnen Schritt für Schritt, wie Sie ein Gemma-Modell mit Ihren eigenen Betriebsdaten trainieren:
...
### 3. Labor 2: Lauffähiger Gemma 4 ReAct-Agent
...
2. Laden Sie das Gemma 4 Modell herunter:
   ```bash
   ollama run gemma4:12b # Oder eine kleinere Variante je nach Hardware
   ```
